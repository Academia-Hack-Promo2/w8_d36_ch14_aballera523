class AccountsController < ApplicationController
	def index
		if Account.all
			render json: Account.all, :except => [:updated_at], :include => [:operations => {:except =>[:updated_at]}]
		else
	...
	def show		
		if Account.exists?(params[:id])			
			render json: Account.find(params[:id]), :except => [:updated_at], :include => [:operations => {:except =>[:updated_at]}]			
		else
	...
	def create
		...
		if cuenta.valid?
			cuenta.save
			render json: cuenta, :except => [:updated_at]
		else
